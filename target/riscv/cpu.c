...
/*
#define get_size_from_bits(ptr, field) \
    ptr->field.size = (1 << ptr->field.bits)
*/

ATTRIBUTE_UNUSED
static void print_dram_element(char *name, dram_element_info *el)
{
    printf("%s: bits:%d-%d-%d off:%d-%d-%d mask:%lx\n", name, el->bits[0], el->bits[1], el->bits[2], el->offsets[0], el->offsets[1], el->offsets[2], el->mask);
}

static void init_default_dram_elements(dram_cpu_info *dram)
{
...
static int parse_dram_el_bits(dram_element_info *el)
{
...
static void read_dram_info_file(RISCVCPU *cpu)
{
    FILE *fi;
    char buff[100];
    char *op;

    memset(&cpu->dram_info, 0, sizeof(dram_cpu_info));

    fi = fopen("dram_struct.txt", "r");
    if(fi == NULL){
        perror("fopen");
        printf("initializing to default DRAM params\n");
        init_default_dram_elements(&cpu->dram_info);
        return;
    }
    ...
    while(fgets(buff, 100, fi) != NULL) {
        ...
        switch(op[0]) {
            case 'c': {
                if(op[1] == 'o') // col
                    el = &cpu->dram_info.col;
                else // chan
                    el = &cpu->dram_info.channel;
                break;
            }
            case 'r': {
                    if(op[1] == 'o') // row
                        el = &cpu->dram_info.row;
                    else
                        el = &cpu->dram_info.rank;
                break;
            }
            case 's':
                    el = &cpu->dram_info.subarr;
                break;

            case 'b':
                    el = &cpu->dram_info.bank;
                break;
...
static void dram_release_timer(void *arg)
{
...
static const TypeInfo riscv_cpu_type_infos[] = {
    {
        ...
        .instance_finalize = riscv_cpu_destroy,
        .abstract = true,
